...
class CustomDdlGenerator {
     ...
     */
    List<String> getDropTableConstraintStatements(String tableName) {
        List<String> result = []
        ...
        try {
            new Sql(connection).eachRow("select CONSTRAINT_NAME from INFORMATION_SCHEMA.TABLE_CONSTRAINTS " +
                ...
                result.add(getDropConstraintStatement(tableName, row.getString('CONSTRAINT_NAME').toUpperCase()))
            }
        } finally {
            connection.close()
        }
     ...
     * <p>
     * Deafult implementation: {@link DdlGeneratorDelegate#defaultGetColumnIndexNames(java.lang.String, java.lang.String)}
     *
     * @param tableName     table name
     * @param columnName    column name
     * @return collection of index names
     */
    Collection<String> getColumnIndexNames(String tableName, String columnName) {
        Set<String> indexes = new LinkedHashSet<>()
        def projectNamespace = delegate.getProjectNamespace()
        def connection = delegate.getConnection()
        try {
            new Sql(connection).eachRow("select distinct i.name, i.is_unique from sys.indexes i " +
                ...
                String indexName = row.getString('NAME').toUpperCase()
                int isUnique = row.getInt('IS_UNIQUE')
                if (isUnique != 1 || !indexName.startsWith("IDX_" + projectNamespace.toUpperCase())) {
     ...
     * <p>
     * Default implementation: {@link DdlGeneratorDelegate#defaultGetColumnConstraintNames(java.lang.String, java.lang.String)}
     *
     * @param tableName     table name
     * @param columnName    column name
     * @return collection of constraint names
     */
    Collection<String> getColumnConstraintNames(String tableName, String columnName) {
        Set<String> constraints = new LinkedHashSet<>()
        String dbName = delegate.getMainDataStoreDatabaseName()
        def connection = delegate.getConnection()
        try {
            new Sql(connection).eachRow("select * from INFORMATION_SCHEMA.CONSTRAINT_COLUMN_USAGE " +
                    "where CONSTRAINT_CATALOG = ? and TABLE_NAME = ? and COLUMN_NAME = ?",
                    [dbName, tableName, columnName]) { row ->
                constraints.add(row.getString("CONSTRAINT_NAME"))
            }
            new Sql(connection).eachRow("select dc.NAME" +
                    " from sys.TABLES t" +
                    " inner join sys.DEFAULT_CONSTRAINTS dc on t.OBJECT_ID = dc.PARENT_OBJECT_ID" +
                    " inner join sys.COLUMNS c on dc.PARENT_OBJECT_ID = c.OBJECT_ID and c.COLUMN_ID = dc.PARENT_COLUMN_ID" +
                    " where t.NAME = ? and c.NAME = ?", [tableName, columnName]) { row ->
                constraints.add(row.getString("NAME"))
            }
        } finally {
            connection.close()
        }
    ...
    Boolean isScaleDifferent(int attributeScale, int columnScale, String columnType) {
        if ('money' == columnType || 'smallmoney' == columnType) {
    ...
    Boolean isTypeDifferent(def attribute, String currentType, String oldType, int oldLength) {
        if (("char" == oldType || "nchar" == oldType) && "varchar" == currentType && oldLength > 1)
            return false
        else
