...
class CustomDbProperties {
    ...
    String getJdbcDriverDependency() {
        return 'com.microsoft.sqlserver:mssql-jdbc:7.2.2.jre8'
    }
    ...
    String createDbUrl(String host, String dbName, String connectionParams) {
        return "${getUrlPrefix()}${host};databaseName=${dbName}${connectionParams}"
    }
    ...
    boolean isNonexistentDatabaseException(SQLException sqlException) {
        return "3D000" == sqlException.getSQLState() || sqlException.getErrorCode() == 4060
    }
    ...
    String getMasterUrl(String host, String dbName, String connectionParams) {
        return createDbUrl(host, "master", connectionParams)
    }
    ...
    String getConnectionParams(String url) {
        ...
        if (m.find()) {
            ...
            def dbNameMatcher = Pattern.compile(/;databaseName=(\w+)/).matcher(params)
            if (dbNameMatcher.find()) {
                params = dbNameMatcher.replaceFirst("")
            }
            def integratedSecurityMatcher = Pattern.compile(/(?i);integratedSecurity=(\w+)/).matcher(params)
            if (integratedSecurityMatcher.find()) {
                params = integratedSecurityMatcher.replaceFirst("")
            }
            return params
        } else {
            return ''
    ...
    String getHost(String url) {
        ...
        if (lastSlash <= 0)
            return "";
        String afterLastSlash = url.substring(lastSlash + 1);
        int firstSemicolon = afterLastSlash.indexOf(';');
        if (firstSemicolon < 0) {
            return afterLastSlash;
        } else {
            return afterLastSlash.substring(0, firstSemicolon);
        }
    }
    ...
    String getDatabaseName(String url) {
        Matcher matcher = Pattern.compile(/;databaseName=(\w+)/).matcher(url);
